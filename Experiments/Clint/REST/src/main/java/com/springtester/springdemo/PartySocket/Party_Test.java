...
public class Party_Test {
    ...
    public Party_Test() throws SQLException {
    }
    ...
    public void turnTest() throws Exception{
        ...
        server.onOpen(sess, "BAM");
        server.onOpen(sess2, "BANGO");

        server.onMessage(sess,"1");
        server.onMessage(sess2,"1");

        assertEquals(0, server.findUsername("BAM"));
        assertEquals(1, server.findUsername("BANGO"));
        assertEquals(2, server.userGroups.size());
        assertEquals(1, server.formattedVals.size());
    }
}